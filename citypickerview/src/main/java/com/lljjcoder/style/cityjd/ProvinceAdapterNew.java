...
public class ProvinceAdapterNew extends BaseAdapter {

    Context context;

    List<ProvinceBeanNew> mProList;

    private int provinceIndex = INDEX_INVALID;

    public ProvinceAdapterNew(Context context, List<ProvinceBeanNew> mProList) {
    ...
    @Override
    public ProvinceBeanNew getItem(int position) {
    ...
    public long getItemId(int position) {
        return Long.parseLong(mProList.get(position).getArea_code());
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ProvinceBeanNew item = getItem(position);
        holder.name.setText(item.getArea_name());

        boolean checked = provinceIndex != INDEX_INVALID && mProList.get(provinceIndex).getArea_name().equals(item.getArea_name());
