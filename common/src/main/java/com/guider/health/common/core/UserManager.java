...
public class UserManager {
    ...
    public void doctorLogin(Context context , String phone , String pass , String dotorAccountId) {
        this.doctorPhone = phone;
        this.doctorPass = pass;
        this.dotorAccountId = dotorAccountId;
        SPUtils.put(context, "doctorPhone", phone);
        SPUtils.put(context , "doctorPass" , pass);
        SPUtils.put(context, "doctorAccountId", dotorAccountId);
    }
    ...
    public void init(JSONObject data) {
        ...
        UserManager.getInstance().setId(data.getInteger("id"));
        UserManager.getInstance().setHeight(data.getInteger("height"));
        UserManager.getInstance().setWeight(data.getInteger("weight"));
        UserManager.getInstance().setAccountId(data.getInteger("accountId"));
    ...
    private void _synchroniseInfo(final Context context , UserInfo userInfo) {
        try {
            ...
            if (!TextUtils.isEmpty(birth)) {
                userInfo.setBirthday(DateUtil.getDateFormat(new SimpleDateFormat("yyyy-MM-dd HH:mm").parse(birth + " 08:00")));
            }
            if (weight > 0) {
