...
import com.inuker.bluetooth.library1.IBluetoothService;
...
import com.inuker.bluetooth.library1.old.connect.response.BleMtuResponse;
import com.inuker.bluetooth.library1.old.connect.response.BleNotifyResponse;
import com.inuker.bluetooth.library1.old.connect.response.BleReadResponse;
import com.inuker.bluetooth.library1.old.connect.response.BleReadRssiResponse;
import com.inuker.bluetooth.library1.old.connect.response.BleUnnotifyResponse;
import com.inuker.bluetooth.library1.old.connect.response.BleWriteResponse;
import com.inuker.bluetooth.library1.old.connect.response.BluetoothResponse;
import com.inuker.bluetooth.library1.old.model.BleGattProfile;
import com.inuker.bluetooth.library1.old.receiver.BluetoothReceiver;
import com.inuker.bluetooth.library1.old.receiver.listener.BleCharacterChangeListener;
import com.inuker.bluetooth.library1.old.receiver.listener.BleConnectStatusChangeListener;
import com.inuker.bluetooth.library1.old.receiver.listener.BluetoothBondListener;
import com.inuker.bluetooth.library1.old.receiver.listener.BluetoothBondStateChangeListener;
import com.inuker.bluetooth.library1.old.receiver.listener.BluetoothStateChangeListener;
...
import com.inuker.bluetooth.library1.old.utils.ListUtils;
import com.inuker.bluetooth.library1.old.utils.proxy.ProxyBulk;
import com.inuker.bluetooth.library1.old.utils.proxy.ProxyInterceptor;
import com.inuker.bluetooth.library1.old.utils.proxy.ProxyUtils;
...
//            BluetoothLog.v(String.format("onServiceDisconnected"));
    ...
    public void requestMtu(String mac, int mtu, final BleMtuResponse response) {
        Bundle args = new Bundle();
        args.putString(Constants.EXTRA_MAC, mac);
        args.putInt(Constants.EXTRA_MTU, mtu);
        safeCallBluetoothApi(Constants.CODE_REQUEST_MTU, args, new BluetoothResponse() {
            @Override
            protected void onAsyncResponse(int code, Bundle data) {
                checkRuntime(true);
                if (response != null) {
                    response.onResponse(code, data.getInt(Constants.EXTRA_MTU, Constants.GATT_DEF_BLE_MTU_SIZE));
                }
            }
        });
    }
