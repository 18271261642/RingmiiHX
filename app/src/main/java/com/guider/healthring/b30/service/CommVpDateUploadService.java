...
import com.guider.healthring.b30.bean.UpHeartBean;
...
//            LogTestUtil.e(TAG, "血压数据" + bpData);
    ...
    private void changeUpload(int type, int position) {
        ...
        switch (type) {
            case STATE_SPORT:
                if (sportData!=null&&sportData.size()>position)dbData = sportData.get(position);
                break;
            case STATE_SLEEP:
                if (sleepData!=null&&sleepData.size()>position)dbData = sleepData.get(position);
                break;
            case STATE_RATE:
                if (rateData!=null&&rateData.size()>position)dbData = rateData.get(position);
                break;
            case STATE_BP:
                if (bpData!=null&&bpData.size()>position)dbData = bpData.get(position);
                break;
        }
