...
//        if (sportData != null) Log.d("------AAA-", "未上传数据条数 运动: " + sportData.size());
//        if (sleepData != null) Log.d("------AAA-", "未上传数据条数 睡眠: " + sleepData.size());
//        if (rateData != null) Log.d("------AAA-", "未上传数据条数 心率: " + rateData.size());
//        if (bpData != null) Log.d("------AAA-", "未上传数据条数 血压: " + bpData.size());
...
//        Log.d("------AAA-步数上传参数", params.toString());
...
//        Log.d("------AAA-----睡眠数据", "uploadSleepData==4==" + dateAndClockForSleep + "==" + dateAndClockForSleep);
        String s = dateToStamp(dateAndClockForSleep);
        String s1 = dateToStamp(dateAndClockForSleep1);
//        Log.d("------AAA--睡眠数据--时间戳", s + "===" + s1);
        ...
        try {
            ...
            //向前1天
            Date dateBeforessddd = H9TimeUtil.getDateBefore(date, 1);
            String nextDayddd = H9TimeUtil.getValidDateStr3(dateBeforessddd);
            Date dateBeforessaaa = H9TimeUtil.getDateBefore(date1, 1);
            String nextDayaaa = H9TimeUtil.getValidDateStr3(dateBeforessaaa);
//            Log.d("times--startTime", nextDayddd + "===endTime" + nextDayaaa);
...
//            params.put("endTime",  sleepData.getSleepUp().getDateAndClockForSleep());
            ...
            params.put("deepLen", "" + sleepData.getDeepSleepTime());
            params.put("shallowLen", "" + sleepData.getLowSleepTime());
            params.put("sleepQuality", "" + sleepData.getSleepQulity());
            params.put("sleepLen", (sleepData.getDeepSleepTime() + sleepData.getLowSleepTime()) + "");
            params.put("sleepCurveP", "" + sleepData.getSleepLine());
            params.put("sleepCurveS", "");
//            MyLogUtil.d("------AAA--睡眠数据---上传参数---", new Gson().toJson(params.toString()));
...
//                Log.d("-----", languageData.getLanguage().toString());
            }
        }
    };
    ...
    private void uploadBpData(int position) {
        if (bpData == null || bpData.isEmpty() || position >= bpData.size()) {
            // 血压数据上传完了,到此结束
//            Log.d("------AAA-", "全部数据上传完成,改变上传状态，没在上传数据------此处设置设备语言");
            //数据上传完咯，改变设备语言
            //判断设置语言
