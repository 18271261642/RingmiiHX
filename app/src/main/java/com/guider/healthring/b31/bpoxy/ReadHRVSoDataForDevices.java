...
public class ReadHRVSoDataForDevices extends AsyncTask<Void, Void, Void> {
    ...
    static boolean UpdataHRV = false;//HRV是否在更新中
    static boolean UpdataO2O = false;//O2O是否在更新中
    ...
    protected void onPreExecute() {
        ...
        MyApp.getContext().registerReceiver(broadcastReceiver, intentFilter);
...
//        Log.e(TAG, "====== 是否只加载今天 " + isToday + "====是否在刷新中 " + isGETDATAS);
    }
    ...
    protected Void doInBackground(Void... voids) {
        Log.e("-------AAA--", "异步任务开始啦 " + isGETDATAS + "--isCancelled()=" + isCancelled());
        // Task被取消了，马上退出循环
...
//        publishProgress();
    ...
    private void readDeviceData() {
        ...
        if (vpOperateManager == null) vpOperateManager = MyApp.getInstance().getVpOperateManager();
        ReadOriginSetting readOriginSetting = new ReadOriginSetting(0, 1, isToday, isToday ? 1 : 2);
        MyApp.getInstance().getVpOperateManager().readHRVOriginBySetting(
                bleWriteResponse,
...
//        Log.e(TAG, "-------isToday=" + isToday);
        ...
        vpOperateManager.readSpo2hOriginBySetting(bleWriteResponse, new ISpo2hOriginDataListener() {
            @Override
            public void onReadOriginProgress(float v) {
//                Log.e(TAG, "---------onReadOriginProgress= " + v + "---value=" + String.valueOf(v));
                //spo2DataProgress =  v;
                if (String.valueOf(v).equals("1.0")) {
                    if (isToday) {
...
//                Log.e(TAG, "---onSpo2hOriginListener " + spo2hOriginData.toString());
