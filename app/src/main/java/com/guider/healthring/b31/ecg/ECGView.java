...
public class ECGView extends View {
    ...
    public ECGView(Context context, AttributeSet attrs, int defStyleAttr)
    {
        ...
        // 心电线的宽度
        heart_line_border = typedArray.getDimensionPixelSize(R.styleable.ECGView_heart_line_border, (int) dip2px(context, 1f));
        // 每个表格的行数（就是小格子数,默认5格
        grid_row = typedArray.getInt(R.styleable.ECGView_heart_grid_row, 5);
        // 大表格的边框的宽度
        heart_grid_border = typedArray.getDimensionPixelSize(R.styleable.ECGView_heart_grid_border, (int) dip2px(context, 2f));
        // 每个小格子的宽高
        grid_row_height = typedArray.getDimensionPixelSize(R.styleable.ECGView_heart_grid_row_height, (int) dip2px(context, 5f));
        // 每个小格子的线的宽度
        heart_grid_line_border = typedArray.getDimensionPixelSize(R.styleable.ECGView_heart_grid_line_border, (int) dip2px(context, 1f));
        // 基准线，默认2000
        baseLine = typedArray.getInteger(R.styleable.ECGView_heart_base_line, 2000);
        // 最大值，默认4000
        Max = typedArray.getInteger(R.styleable.ECGView_heart_max, 4096);
        // 最小值，默认0
        Min = typedArray.getInteger(R.styleable.ECGView_heart_min, 0);
        // 数据采集频率，默认100个点一秒钟
        hz = typedArray.getInteger(R.styleable.ECGView_heart_hz, 250);
        // 一个控件，可以显示的心率的时长 ,默认为2秒钟
        showSeconds = typedArray.getFloat(R.styleable.ECGView_heart_show_seconds, 2f);
        // 心率线条的颜色 默认红色
        heartColor = typedArray.getColor(R.styleable.ECGView_heart_color, Color.RED);
        // 表格线条的颜色 默认绿色
        heart_grid_line_color = typedArray.getColor(R.styleable.ECGView_heart_grid_line_color, Color.parseColor("#DBDBDB"));
        // 表格边框的颜色 默认绿色
        heart_grid_border_color = typedArray.getColor(R.styleable.ECGView_heart_grid_border_color, Color.parseColor("#DBDBDB"));
        // 播放速度的控制
        heart_speed = typedArray.getFloat(R.styleable.ECGView_heart_speed, 1.0f);
        typedArray.recycle();
