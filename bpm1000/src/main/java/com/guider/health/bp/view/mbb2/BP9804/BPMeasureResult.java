...
public class BPMeasureResult extends BPFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.bp_mbb_meassure_result, container, false);
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        int dbp = Integer.parseInt(HeartPressMbb_9804.getInstance().getDbp());
        int sbp = Integer.parseInt(HeartPressMbb_9804.getInstance().getSbp());

        setStandard();

        ((TextView) view.findViewById(R.id.bp_press1)).setText(sbp+"");
        ((TextView) view.findViewById(R.id.bp_press2)).setText(dbp+"");
        ((TextView) view.findViewById(R.id.bp_result)).setText(HeartPressMbb_9804.getInstance().getSbp() + "/" + HeartPressMbb_9804.getInstance().getDbp());
    ...
    private void setStandard() {
        HeartPressMbb_9804.getInstance().startStandardRun(new StandardCallback() {
            @Override
            public void onResult(boolean isFinish) {
                ...
                if (isFinish) {
                    if ("理想血压".equals(HeartPressMbb_9804.getInstance().getCardShowStr())){
        ...
        MeasureDataUploader.getInstance(_mActivity).uploadBP(
                HeartPressMbb_9804.getInstance().getDeviceAddress(),
                HeartPressMbb_9804.getInstance().getDbp(),
                HeartPressMbb_9804.getInstance().getSbp(),
                HeartPressMbb_9804.getInstance().getHeart()
        );
        MeasureDataUploader.getInstance(_mActivity).uploadHeartBpm(
                Integer.valueOf(HeartPressMbb_9804.getInstance().getHeart())
        );
    }
    ...
    private void setLocal() {
        ...
        paramHealthRangeAnlysis1.setValue1(HeartPressMbb_9804.getInstance().getSbp());
        paramHealthRangeAnlysis1.setValue2(HeartPressMbb_9804.getInstance().getDbp());
        ...
        if (results != null){
            HeartPressMbb_9804.getInstance().setCardShowStr(results.get(0));
            HeartPressMbb_9804.getInstance().setCardShowStr(results.get(0));
        }
