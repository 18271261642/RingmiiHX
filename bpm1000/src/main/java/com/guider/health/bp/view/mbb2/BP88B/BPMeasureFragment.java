...
public class BPMeasureFragment extends BPFragment implements Protocol.IView {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.bp_mbb88_connect_and_meassure, container, false);
        return view;
    }
...
//        Toast.makeText(_mActivity, power, Toast.LENGTH_SHORT).show();
    ...
    public void onMeasureResult(MeasurementResult result) {
        //测量结果
        HeartPressMbb_88.getInstance().setSbp(result.getCheckShrink() + "");
        HeartPressMbb_88.getInstance().setDbp(result.getCheckDiastole() + "");
        HeartPressMbb_88.getInstance().setHeart(result.getCheckHeartRate() + "");
