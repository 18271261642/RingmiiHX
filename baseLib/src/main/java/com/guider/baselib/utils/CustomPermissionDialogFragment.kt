...
import kotlinx.android.synthetic.main.custom_permission_dialog_layout.*
...
//            Manifest.permission.ACCESS_MEDIA_LOCATION to Manifest.permission_group.STORAGE
    ...
    override fun onCreateView(
        ...
        return LayoutInflater.from(context).inflate(R.layout.custom_permission_dialog_layout,
                container, false)
    }
    ...
    private fun buildPermissionsLayout() {
        for (permission in mPermissions) {
            ...
            if (permissionGroup != null && !groupSet.contains(permissionGroup)) {
                val textView = LayoutInflater.from(context).inflate(R.layout.permissions_item,
                        permissionsLayout, false) as TextView
                textView.text = context?.let {
                    it.packageManager.getPermissionGroupInfo(permissionGroup,
                            0).loadLabel(it.packageManager)
                }
