...
public class FORABodyWeightScaleUtil extends FORA {
	...
	public void setOnFORADateFormatTool(BodyWeightScaleTool tool){
		this.tool = tool;
	}
	...
	public void measureDataFormat(ArrayList<Byte> characteristicValues, 
								  BluetoothGattCharacteristic characteristic, 
								  BluetoothGatt gatt) {		
		System.out.println("size:"+characteristicValues.size());
		if (characteristic.getValue() != null & characteristicValues.size() == 33) {	
//			int fat = intParse(characteristic, 0);
//			int fatMultiplier = intParse(characteristic, 0);	
//			setFat(String.valueOf(fat));
			int weightRemainder= intParse(characteristicValues, 16);	
			int weightMultiplier = intParse(characteristicValues, 17);	
			int bmiMultiplier = intParse(characteristicValues, 21);	
			int bmiRemainder = intParse(characteristicValues, 22);
			double weight = (double) weightCompute(weightRemainder, weightMultiplier);
			double bmi = (double) weightCompute(bmiRemainder, bmiMultiplier);
			
			Log.e("weightMultiplier", weightMultiplier+"");
			Log.e("weightRemainder", weightRemainder+"");
			Log.e("bmiMultiplier", bmiMultiplier+"");
			Log.e("bmiRemainder", bmiRemainder+"");
			
			setWeight(String.valueOf(weight));		
			setBmi(String.valueOf(bmi));	
...
//		httpParams.append("Day=").append(params[3]).append(separator);
		...
		httpParams.append("DataType=").append("8").append(separator);
		httpParams.append("Value1=").append(params[6]).append(separator);
		httpParams.append("Value2=").append(params[7]).append(separator);
//		httpParams.append("Value3=").append(params[8]).append(separator);
	...
	private double weightCompute(double remainder, double multiplier) {
		if (remainder == 0)
			return multiplier / 10;			
		else
			return ((256 * remainder) + multiplier) / 10;
	}
