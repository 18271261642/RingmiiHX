...
//                getRunMapListData(runTags);    //获取地图的历史记录
    ...
    void findDatas() {
        ...
        String bm1 = (String) SharedPreferenceUtil.get(MyApp.getContext(), Commont.BLEMAC, null);
        ...
        final List<B30DevicesSport> b30DevicesSports = MyApp.getDBManager().getDaoSession().getB30DevicesSportDao()
                .queryBuilder().where(B30DevicesSportDao.Properties.Address.eq(TextUtils.isEmpty(bm1) ? bm : bm1)
        ...
        if (!b30DevicesSports.isEmpty()) {
            ...
            if (list_kcluli != null) list_kcluli.setAdapter(new BaseAdapter() {
                ...
                public View getView(int i, View view, ViewGroup viewGroup) {
                    ...
                    textViewTimes.setText( Constant.secToTime(b30DevicesSports.get(i).getSportTime()));
...
//                                + "\n心率:" + sportModelOriginHeadData.getAverRate());
                ...
                List<B30DevicesSport> b30DevicesSports = MyApp.getDBManager().getDaoSession().getB30DevicesSportDao().loadAll();
                if (b30DevicesSports == null || b30DevicesSports.size() <= 0) {
                    MyApp.getDBManager().getDaoSession().getB30DevicesSportDao().insert(db);
                } else {
                    for (int i = 0; i < b30DevicesSports.size(); i++) {

                        B30DevicesSport unique = MyApp.getDBManager().getDaoSession().getB30DevicesSportDao().queryBuilder().where(B30DevicesSportDao.Properties.StartTime.eq(sportModelOriginHeadData.getStartTime().getYear()
                                + "-" + sportModelOriginHeadData.getStartTime().getMonth()
                                + "-" + sportModelOriginHeadData.getStartTime().getDay()
                                + " " + sportModelOriginHeadData.getStartTime().getHour()
                                + ":" + sportModelOriginHeadData.getStartTime().getMinute()
    ...
    private void getRunLiatDataDB(int runTags, String dataTime) {
        ...
        if(daoSession == null)
            return;
        final List<SportMaps> sportMapsList = daoSession.getSportMapsDao()
