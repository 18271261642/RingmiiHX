...
import com.guider.healthring.w30s.utils.ScreenUtils;
...
//        barMores.setVisibility(View.GONE);
    ...
    private void getSleepData(String timeDatas) {
        ...
        if (Commont.isDebug)Log.d("TAG","---------timeData----------"+ timeDatas);
        ...
        if (Commont.isDebug)Log.d("----timeData---", WatchUtils.getCurrentDate().substring(0, 7));
        //tring类型转换为date类型
        ...
        try {
            ...
            sleepJson.put("deviceCode", (String) SharedPreferenceUtil.get(SleepHistoryActivity.this, Commont.BLEMAC, ""));
            if (isToMoth) {
...
//        OkHttpObservable.getInstance().getData(commonSubscriber, sleepUrl, sleepJson.toString());
    ...
    public class SleepDataAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (Commont.isDebug)Log.d("--->" + this.getClass(), "总共长度：" + size + "时间：" + rtc + "深睡：" + deepSleep + "浅睡：" + shallowSleep + "清醒：" + soberLen + "====" + sleepLen + "====" + weekCount);
