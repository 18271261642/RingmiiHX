...
//        publishProgress();
    ...
    private void findNotUploadData() {
        ...
        if (instance !=null){
            ...
            if (sportData != null&&!sportData.isEmpty()) Log.e(TAG, "未上传数据条数 运动: " + sportData.size());
            if (sleepData != null&&!sleepData.isEmpty()) Log.e(TAG, "未上传数据条数 睡眠: " + sleepData.size());
            if (rateData != null&&!rateData.isEmpty()) Log.e(TAG, "未上传数据条数 心率: " + rateData.size());
            if (bpData != null&&!bpData.isEmpty()) Log.e(TAG, "未上传数据条数 血压: " + bpData.size());
    ...
    private void loginGdServices() {
        ...
        //Log.e(TAG, "游客注册或者登陆" + params.toString());
...
//                }, true);
        ...
        OkHttpTool.getInstance().doRequest(URLs.GDHTTPS + "/SendCardId",
                params, this, loginHttpResult, true);
    }
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                if (allStep > 0) {
                    ...
                    if (SportCountAdd < SportCount) {
                        ...
                        Log.e(TAG, "sport- 上传步数参数:" + params.toString());
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                if (rateValue > 0) {
                    ...
                    if (HeartCountAdd < HeartCount) {
                        ...
                        params.put("CARDID", deviceCode);
                        params.put("CDATE", heartDates.replace(":", "-"));
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                if (highValue > 0 && lowValue > 0) {
                    ...
                    if (BpCountAdd < BpCount) {
                        ...
                        params.put("HB", "0");
                        params.put("CARDID", deviceCode);
                        params.put("CDATE", bpDates.replace(":", "-"));
...
//                    Log.e(TAG, "时间对比--D-----" + currentDate + "=====" + dbData.getDate());
    ...
    private boolean resultSuccess(String result) {
        try {
            ...
            if (resultVo != null) {
                if (resultVo.getSTATUS()!= null && resultVo.getSTATUS().equals(RESULT_CODE)) {
