...
public class NewB30BloadDataView extends View {
    ...
    public NewB30BloadDataView(Context context) {
        super(context);
    }

    public NewB30BloadDataView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
    }

    public NewB30BloadDataView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        float start = getWidth() / mapList.size();
        ...
        float auto = getHeight() / 250;//将高度平均分250个刻度
        //绘制连线
        Path path = new Path();
        if (mapList != null && mapList.size() > 0) {
            for (int i = 0; i < mapList.size(); i++) {
...
//                float xPoint = start * i - mCirRadio / 2;
                if ((-intArray.keyAt(0)) != 0) {//判断，去除为0是所绘制的点
                    Log.e(TAG,"低压点===   "+intArray.valueAt(0)+"   高压点===   "+intArray.keyAt(0));
